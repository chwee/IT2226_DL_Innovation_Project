...
- **Version**: 1.1 (updated for Requirement 3's read-aloud MUST change — see Change Log)
- **Date**: August 11, 2026
- **Author**: Wee Chee Hong
- **Reviewers**: Course instructor (ITI122)
- **Related Documents**: `requirements_app.md` (v1.1)

### Change Log
- **2026-08-11 (v1.1)**: Read-aloud (Requirement 3, AC 3.2/3.3/3.5) is now a MUST, not optional. Component 1's responsibilities below updated: the read-aloud control is always rendered after a successful result; browser support is feature-detected only at activation time (to choose "speak" vs. "show a clear error"), never to decide whether the control is shown.
...
- **Browser-native read-aloud, not server-generated audio.** Requirement 3's assumption specifies the browser's built-in speech synthesis (Web Speech API) rather than a server-side text-to-speech call. This avoids adding an audio-generation dependency and an extra round trip. Read-aloud is now a MUST (v1.1): the control's *visibility* no longer depends on feature detection — it is always shown after a successful result — while the API's availability is still feature-detected, but only at activation time, to decide between speaking the text and showing a clear inline error (AC 3.5).
...
- Always render the read-aloud control after a successful result, regardless of browser capability (Requirement 3, AC 3.2 — MUST). Feature-detect the browser's Web Speech API only at activation time: play the translated text as speech when supported (AC 3.3), or display a clear inline error when not (AC 3.5) — never hide or disable the control itself.
