...
A web application that detects the source language of user-entered text and translates it into a user-selected target language, displaying both the original and translated text with an optional read-aloud function.
...
**In scope:** single text-entry translation, automatic source-language detection, target-language selection, displaying source + translated text, optional read-aloud of the translated text.
...
**User Story:** As a user, I want to see both my original text and the translation, and optionally hear the translation read aloud, so that I can verify and use the result.
...
2. WHEN a translated result is displayed THEN the Frontend SHALL show a read-aloud control alongside it.
3. WHEN the user activates the read-aloud control THEN the Frontend SHALL play back the translated text as speech.
...
5. WHERE the read-aloud feature is unavailable in the user's browser THEN the Frontend SHALL hide or disable the read-aloud control rather than fail silently.
...
- **Assumptions**: Read-aloud is implemented using the browser's built-in speech synthesis capability rather than a server-generated audio file.
...
1. WHEN a user submits a translation request under normal conditions THEN the Backend SHALL return a response within 3 seconds.
2. IF the text input exceeds a reasonable length (e.g., 5,000 characters) THEN the Backend SHALL reject the request with a 400 error rather than attempt a slow or unbounded translation.
...
- IF the read-aloud feature is not supported by the browser THEN the Frontend SHALL clearly indicate that the feature is unavailable rather than showing a non-functional control.
